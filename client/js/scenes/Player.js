class Player extends Phaser.Physics.Arcade.Sprite {
    ...
    showChatMessage(message) {
        ...
        this.chatText = this.scene.add.text(this.x, this.y - 60, message, {
            fontSize: '16px',
            color: '#fff',
            backgroundColor: '#000',
            padding: { x: 6, y: 2 }
    ...
    update(cursors, wasd) {
        ...
        // Обновляем позицию никнейма и чата
        if (this.nicknameText) {
            this.nicknameText.setPosition(this.x, this.y - 35);
        }
        if (this.chatText) {
            this.chatText.setPosition(this.x, this.y - 60);
        }
    }
